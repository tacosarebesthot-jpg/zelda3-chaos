...
bool ZeldaRunFrame(int inputs) {
  ...
  // v1 provisional settings entry (F12): opens the shared settings menu with
  // the randomizer rows registered from randomizer.c. Owner may replace this
  // presentation in v2 (ZALiA-style pause page).
  Randomizer_SettingsTick();

  frame_ctr_dbg++;

  bool is_replay = state_recorder.replay_mode;

  // Twitch chat integration: input mutations + command drain + effect timers.
  // Never during a replay: chat effects spawn sprites and edit health and
  // inventory, which diverges the replay from the recorded run and fails
  // every snapshot compare.
  if (!is_replay) {
    inputs = Twitch_PreFrame(inputs);
    Twitch_Tick();
  }
  ...
  if (is_replay) {
    ...
    if (animated_tile_data_src != 0) {
      ...
      // FLASHING (the "reduce_flashing" row): OR the engine's existing global
      // DimFlashes feature bit in on top of whatever zelda3.ini says, for a
      // file with the row on REDUCED - see Randomizer_FlashingReduced.  This
      // is recomputed every check (not just once at file load), so it survives
      // Video_Apply() resetting g_wanted_zelda_features from g_config.features0
      // whenever the OPTIONS page applies something unrelated mid-file.
      uint32 wanted_features = g_wanted_zelda_features;
      if (Randomizer_FlashingReduced())
        wanted_features |= kFeatures0_DimFlashes;
      if (enhanced_features0 != wanted_features) {
        enhanced_features0 = wanted_features;
