...
void Module00_Intro() {  // 8cc120
  // stream build: with SkipIntroOnKeypress any button from the very first
  // intro frame (after init, submodule 2) jumps to file select - no logo,
  // no triforce spin.  Vanilla waits for the title (8).
  uint8 skip_at = enhanced_features0 & kFeatures0_SkipIntroOnKeypress ? 2 : 8;

  if (submodule_index >= skip_at && ((filtered_joypad_L & 0xc0 | filtered_joypad_H) & 0xd0)) {
    if (Twitch_Debug())
      printf("RESULT intro skipped sub=%d fL=%02x fH=%02x L=%02x H=%02x\n", submodule_index,
             filtered_joypad_L, filtered_joypad_H, joypad1L_last, joypad1H_last), fflush(stdout);
...
void Credits_AddNextAttribution() {  // 8ebe24
  ...
  if (R18 < 394) {
    ...
    {
      const uint8 *community = Credits_CommunityRow(R18);   // stream build: chatters in the roll
      if (community) src = community;
    }
    if (*src != 0xff) {
      *dst++ = swap16(R16 + *src++);
      int n = *src++;
