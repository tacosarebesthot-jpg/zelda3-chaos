...
// MODS page state (the page itself lives further down, before Hud_NormalMenu)
static int g_hud_mods_page;    // 0 = inventory page, 1 = MODS page
static int g_hud_mods_cursor;  // highlighted MODS row
static int s_hud_sel_held;     // SELECT hold tracker for the save/exit gate
static int s_hud_mods_hotkey;  // KeyMap ModsPage pressed; consumed by Hud_NormalMenu
void Hud_ModsPageHotkey() { s_hud_mods_hotkey = 1; }
static int s_hud_sel_chord;    // the SELECT hold was a MODS chord
...
void Hud_RefillLogic() {  // 8ddb92
  ...
  // BOMB BAG: Randomizer_MaxBombs() is the vanilla kMaxBombsForLevel lookup
  // everywhere except on a bomb-bag file that has not found the bag, where it
  // is 0 - so the refill stops and anything already held is dropped.  Vanilla
  // and every non-bomb-bag file take exactly the old path.
  {
    int maxbombs = Randomizer_MaxBombs();
    if (maxbombs == 0) {
      link_item_bombs = 0;
      link_bomb_filler = 0;
    } else if (link_bomb_filler) {
      link_bomb_filler--;
      if (link_item_bombs != maxbombs)
        link_item_bombs++;
    }
  }
  ...
  if (!flag_is_link_immobilized && !link_hearts_filler &&
    ...
    if (link_lowlife_countdown_timer_beep) {
      ...
      // HEART BEEP (randomizer.c): 0 = the row is OFF, never play the sound
      // effect, but still reload the timer so turning the row back on mid
      // file needs no extra state.  32 (vanilla) reproduces the old literal
      // exactly.
      int beep_interval = Randomizer_HeartBeepInterval();
      if (beep_interval && !(enhanced_features0 & kFeatures0_DisableLowHealthBeep))
        sound_effect_1 = 43;
      link_lowlife_countdown_timer_beep = (uint8)((beep_interval ? beep_interval : 32) - 1);
    }
  }
...
void Hud_Module_Run() {  // 8ddd36
  byte_7E0206++;
  // entering the pause-menu state machine always starts clean: exotic paths
  // (save&quit, restore, replays) can bypass Hud_CloseMenu, so scrub the
  // MODS page state on the entry states - they run before anything reads it
  if (overworld_map_state == 0 || overworld_map_state == 1) {
    g_hud_mods_page = 0;
    g_hud_mods_cursor = 0;
    s_hud_sel_held = 0;
    s_hud_sel_chord = 0;
...
void Hud_Init() {  // 8dddab
  // every pause-menu open starts on the inventory page (and with no stale
  // SELECT-chord state); Hud_CloseMenu resets these too
  g_hud_mods_page = 0;
  g_hud_mods_cursor = 0;
  s_hud_sel_held = 0;
  s_hud_sel_chord = 0;
...
// MENU SPEED (randomizer.c Randomizer_MenuSpeedSteps): how many vanilla
// 8-unit BG3VOFS steps to take this frame, or "skip this call" for HALF.
// See the comment on Randomizer_MenuSpeedSteps for why the step size (8)
// itself is never touched.
static int Hud_MenuSpeedStepsThisCall(void) {
  static uint8 s_half_toggle;
  int v = Randomizer_MenuSpeedSteps();
  if (v != -1)
    return v;
  s_half_toggle ^= 1;
  return s_half_toggle;
}

void Hud_BringMenuDown() {  // 8dde59
  int steps = Hud_MenuSpeedStepsThisCall();
...
// ---- v1 PROVISIONAL MODS page (owner may replace presentation in v2) ----
// Gesture: HOLD SELECT + tap L/R shoulder = flip inventory <-> MODS page
// (ALTTPR-style two-page pause menu).  UP/DOWN move the mods cursor,
// LEFT/RIGHT adjust the selected setting, A activates; B/Start still exit.
// Text: the HUD font is pre-encoded tile words, not ASCII - caps A-Z are
// consecutive tiles (glyph 0x50+n of the vram 0x7800 font -> tile 0x150+n,
// verified against the ability-box words LIFT/READ/TALK/PULL/RUN/SWIM) and
// digits sit at tile 0x127+d (inverse-video chips, see "Lift1"/"Lift2").
#define kMods_SelectHeld (joypad1H_last & kJoypadH_Select)
#define kMods_LRchord (filtered_joypad_L & (kJoypadL_L | kJoypadL_R))
// either order works: SELECT held + tap L/R, or L/R held + press SELECT.
// Pads made the second one the natural one (SELECT release alone opens the
// save prompt, so people hold the shoulder first).
#define kMods_SelectEdge (filtered_joypad_H & kJoypadH_Select)
#define kMods_LRheld (joypad1L_last & (kJoypadL_L | kJoypadL_R))
enum {
  kModsRow_Twitch = 0,   // chat status; A: copy the connect link to the clipboard
  kModsRow_Randomizer,   // ON/OFF - randomizer.ini, applies next boot
  kModsRow_Seed,         // LEFT/RIGHT adjust - randomizer.ini, next boot
  kModsRow_Logic,        // LEFT/RIGHT cycle the glitch-logic tier, next boot
  kModsRow_Rewards,      // A: the channel-points REWARDS page (map reward -> verb)
  kModsRow_Sprite,       // LEFT/RIGHT/A: cycle the sprite library, live
  kModsRow_Text,         // LEFT/RIGHT/A: NORMAL / CLEAN / DIRTY jokes (dialogue.txt), live
  kModsRows
};
static int g_hud_mods_sub;      // 0 = MODS rows, 1 = REWARDS page
static int g_hud_rew_cursor;    // highlighted reward on the REWARDS page
// defined further down in this file; needed to repaint the inventory page
// after the MODS page blanked over it
static void Hud_DrawBox(uint16 *dst, int x1, int y1, int x2, int y2, uint8 palette);
void Hud_DrawYButtonItems();
void Hud_DrawAbilityBox();
void Hud_DrawProgressIcons();
void Hud_DrawSelectedYButtonItem();

// One HUD-font text cell for the mods page: caps and digits only, every
// other character blanks.  Palette 3 + priority bit, the same word style as
// the ability-box text (0x2d5b etc).
// Queue the standard BG3 tilemap upload (uvram_screen, target 0x22) - but
// ONLY when the NMI dispatcher is idle.  NMI clears nmi_subroutine_index
// after running it, so during pause-menu frames this is always idle and the
// upload fires exactly like vanilla; a task queued by some other subsystem
// (messaging/dialogue = 2, cutscene gfx = 5/7/8/12/13/21/22...) is never
// stomped, so a MODS-page frame can never hijack a non-hud NMI transfer.
static void Hud_QueueTilemapUpload() {
  if (nmi_subroutine_index == 0) {
    nmi_subroutine_index = 1;
    BYTE(nmi_load_target_addr) = 0x22;
  }
}
static uint16 g_hud_mods_pal = 0x2c00;   // 0x2c00 white, 0x3400 yellow (selected row)
static uint16 Hud_ModsGlyph(char c) {
  if (c >= 'A' && c <= 'Z')
    return g_hud_mods_pal | (0x150 + (c - 'A'));
  // digits: the HUD counter glyphs (rupees/bombs/arrows use tile 0x90+d
  // with palette 1, see Hud_IntToDecimal). The 0x127+d "chip" tiles only
  // exist for 1-3 (LIFT.1 etc.) and rendered 4-9 as item icons.
  if (c >= '0' && c <= '9')
    return 0x2400 | (0x90 + (c - '0'));
  return 0x2cf5;
}

static void Hud_ModsText(uint16 *dst, int x, int y, const char *s) {
  for (int i = 0; s[i] != 0 && x + i < 30; i++)
    dst[HUDXY(x + i, y)] = Hud_ModsGlyph(s[i]);
}

// Right-aligned to end at column xe (the page's values column).
static void Hud_ModsTextRight(uint16 *dst, int xe, int y, const char *s) {
  int n = 0;
  while (s[n] != 0) n++;
  Hud_ModsText(dst, xe + 1 - n, y, s);
}

// Right-aligned decimal number; v <= 0 renders RANDOM (0 = time-derived seed).
static void Hud_ModsNum(uint16 *dst, int xe, int y, int v) {
  if (v <= 0) {
    Hud_ModsTextRight(dst, xe, y, "RANDOM");
    return;
  }
  char buf[10];
  int n = 0;
  ...
  Hud_ModsTextRight(dst, xe, y, &buf[9 - n]);
}

// Pending randomizer edits made on this page that randomizer.ini does not
// have yet.  The RANDOMIZER toggle commits immediately (one write per tap,
// same as the file-select row); SEED marks dirty and commits once the
// d-pad / shoulder is released or the menu closes, so hold-repeat and the
// +-100 shoulder steps do not rewrite the ini every frame.
static int s_hud_mods_seed_dirty;
static int s_hud_mods_hold_frames;  // frames LEFT/RIGHT has been held

static void Hud_ModsCommitSeedIfDirty() {
  if (s_hud_mods_seed_dirty) {
    s_hud_mods_seed_dirty = 0;
    Randomizer_SettingsCommit();
  }
}

// LEFT = -1, RIGHT = +1, A button = 0 (activate).  Mirrors the semantics of
// the settings rows registered by oauth_web.c/randomizer.c/sprites.c.
// Every randomizer edit reaches randomizer.ini: the file-select rows already
// committed per change; this page used to leave the edits in memory only.
static void Hud_ModsAdjust(int delta) {
  switch (g_hud_mods_cursor) {
  case kModsRow_Twitch:
    if (delta == 0)
      OAuthWeb_CopyConnectLink();  // prints + clipboard-copies the URL
    break;
  case kModsRow_Rewards:
    if (delta == 0) {
      g_hud_mods_sub = 1;
      g_hud_rew_cursor = 0;
      sound_effect_2 = 32;
    ...
    break;
  case kModsRow_Randomizer:
  case kModsRow_Seed:
  case kModsRow_Logic:
    // fixed when the file was made (new-file setup step on the file select)
    sound_effect_2 = 0x3c;
    break;
  case kModsRow_Sprite:
    Sprite_Select_Cycle(delta == 0 ? 1 : delta);
    break;
  case kModsRow_Text:
    DialogueOverride_Toggle();
    break;
  default:  // kModsRow_Twitch: nothing to adjust
    break;
  }
}

static void Hud_ModsPage() {
  uint16 *dst = uvram_screen.row[0].col;
  const int y0 = 3;
  // paint over the inventory page: blank the layer, then box chrome
  for (int y = y0; y < y0 + 25; y++)
    for (int x = 1; x < 31; x++)
      dst[HUDXY(x, y)] = 0x207f;
  Hud_DrawBox(dst, 1, y0, 30, y0 + 24, 1);   // bottom border on the last screen row
  Hud_ModsText(dst, 4, y0 + 1, "MODS");
  // COLLECT RATE: the seed's found/total, right-aligned on the title row (the
  // seven rows below are the selectable ones and are all spoken for).  Counted
  // over the chest placements the fill actually wrote - the only spots whose
  // "taken" state the engine can read back - so it is a chest rate, not the
  // reference's 216-location rate.
  {
    int got, total;
    if (Randomizer_CollectionRate(&got, &total)) {
      char v[24];
      snprintf(v, sizeof(v), "ITEMS %d/%d", got, total);
      Hud_ModsTextRight(dst, 28, y0 + 1, v);
    }
  }
  // TRIFORCE HUNT (GOAL = TRIFORCE): pieces found of the number that ends the
  // game.  Same title row, left of the collect rate: "MODS" ends at column 8
  // and "ITEMS nnn/nnn" is right-aligned at 28, so columns 10..17 are free.
  {
    int goal = Randomizer_TriforceGoal();
    if (goal) {
      char v[16];
      snprintf(v, sizeof(v), "TRI %d/%d", link_triforce_pieces, goal);
      Hud_ModsText(dst, 10, y0 + 1, v);
    }
  }

  // row labels at y = 6, 9, 12, 15, 18 (3 apart), values right-aligned to 28
  Hud_ModsText(dst, 4, 6, "TWITCH");
  if (OAuthWeb_SaveState() == kOAuthSavePending)
    Hud_ModsTextRight(dst, 28, 6, "PENDING");
  else
    Hud_ModsTextRight(dst, 28, 6, Twitch_IsConnected() ? "ONLINE" : "OFFLINE");

  Hud_ModsText(dst, 4, 9, "RANDOMIZER");
  Hud_ModsTextRight(dst, 28, 9, Randomizer_FileEnabled() ? "ON" : "OFF");

  Hud_ModsText(dst, 4, 12, "SEED");
  Hud_ModsNum(dst, 28, 12, Randomizer_FileSeed());

  // Glitch-logic tier (randomizer.ini logic=), applies next boot
  Hud_ModsText(dst, 4, 15, "LOGIC");
  Hud_ModsTextRight(dst, 28, 15, Randomizer_LogicLabelFor(Randomizer_FileLogic()));

  // Channel points: status + how many rewards are known; A opens the page
  Hud_ModsText(dst, 4, 18, "REWARDS");
  {
    char v[24];
    int n = CP_RewardCount();
    if (n > 0)
      snprintf(v, sizeof(v), "%s %d", CP_StatusText(), n);
    else
      snprintf(v, sizeof(v), "%s", CP_StatusText());
    Hud_ModsTextRight(dst, 28, 18, v);
  }

  // Applied sprite name (sprites.c exports it; "VANILLA" when none).  The
  // value column runs from 12 to 28, so cap at 16 glyphs to keep clear of
  // the SPRITE label.
  Hud_ModsText(dst, 4, 21, "SPRITE");
  {
    const char *name = Sprite_Select_CurrentName();
    char short_name[17];
    int n = 0;
    while (name[n] != 0 && n < 16) { short_name[n] = name[n]; n++; }
    short_name[n] = 0;
    Hud_ModsTextRight(dst, 28, 21, short_name);
  }
  // In-game text: JOKES = dialogue.txt overrides, NORMAL = original script
  Hud_ModsText(dst, 4, 24, "TEXT");
  Hud_ModsTextRight(dst, 28, 24, !DialogueOverride_Available() ? "NO FILE" : DialogueOverride_ModeLabel());
  // one-line hint on the bottom row of the box
  Hud_ModsText(dst, 3, 26, g_hud_mods_cursor == kModsRow_Twitch ? "A COPIES THE CONNECT LINK" :
                            g_hud_mods_cursor == kModsRow_Rewards ? "A OPENS THE REWARDS PAGE" :
                            (g_hud_mods_cursor == kModsRow_Randomizer || g_hud_mods_cursor == kModsRow_Seed ||
                             g_hud_mods_cursor == kModsRow_Logic) ? "FIXED WHEN THE FILE WAS MADE" :
                            "                          ");

  // cursor: the selected row's label in the HUD's yellow (the old blank-tile
  // band was invisible - the tile is empty in this palette)
  {
    static const char *const kLabels[kModsRows] = {
      "TWITCH", "RANDOMIZER", "SEED", "LOGIC", "REWARDS", "SPRITE", "TEXT" };
    g_hud_mods_pal = 0x3400;
    Hud_ModsText(dst, 4, 6 + g_hud_mods_cursor * 3, kLabels[g_hud_mods_cursor]);
    g_hud_mods_pal = 0x2c00;
  }

  // input: UP/DOWN cursor, LEFT/RIGHT adjust (tap = 1, hold = 10 every 4
  // frames after a 20-frame delay), L/R shoulder on the SEED row = +-100
  // (a bare shoulder tap - SELECT is not held, so it is not the page chord),
  // A activates.  Seed 1234 from 1 is now ~12 shoulder taps instead of 1233.
  int lr_held = joypad1H_last & (kJoypadH_Left | kJoypadH_Right);
  if (filtered_joypad_H & kJoypadH_Up) {
    g_hud_mods_cursor = (g_hud_mods_cursor + kModsRows - 1) % kModsRows;
  } else if (filtered_joypad_H & kJoypadH_Down) {
    g_hud_mods_cursor = (g_hud_mods_cursor + 1) % kModsRows;
  } else if (filtered_joypad_H & (kJoypadH_Left | kJoypadH_Right)) {
    s_hud_mods_hold_frames = 0;
    Hud_ModsAdjust((filtered_joypad_H & kJoypadH_Left) ? -1 : 1);
  } else if (lr_held && g_hud_mods_cursor == kModsRow_Seed) {
    if (++s_hud_mods_hold_frames >= 20 && (s_hud_mods_hold_frames & 3) == 0)
      Hud_ModsAdjust((lr_held & kJoypadH_Left) ? -10 : 10);
  } else if ((filtered_joypad_L & (kJoypadL_L | kJoypadL_R)) &&
             g_hud_mods_cursor == kModsRow_Seed) {
    Hud_ModsAdjust((filtered_joypad_L & kJoypadL_L) ? -100 : 100);
  } else if (filtered_joypad_L & kJoypadL_A) {
    Hud_ModsAdjust(0);
  }
  if (!lr_held) {
    s_hud_mods_hold_frames = 0;
    if (!(joypad1L_last & (kJoypadL_L | kJoypadL_R)))
      Hud_ModsCommitSeedIfDirty();  // nothing held: flush the seed edit
  }
}

// REWARDS page: every channel-point reward Twitch reported (or that has been
// redeemed), one per row, with the verb it triggers.  LEFT/RIGHT change the
// verb (saved at once, cp_rewards.ini), UP/DOWN move, B goes back.  Eight
// rows per page.  Nothing to edit by hand, no names anywhere.
#define kRewRows 8
static void Hud_RewardsPage() {
  uint16 *dst = uvram_screen.row[0].col;
  const int y0 = 3;
  for (int y = y0; y < y0 + 22; y++)
    for (int x = 1; x < 31; x++)
      dst[HUDXY(x, y)] = 0x207f;
  Hud_DrawBox(dst, 1, y0, 30, y0 + 21, 1);
  Hud_ModsText(dst, 4, y0 + 1, "REWARDS");
  Hud_ModsTextRight(dst, 28, y0 + 1, CP_StatusText());

  int n = CP_RewardCount();
  if (n == 0) {
    Hud_ModsText(dst, 4, 9, "NO REWARDS YET");
    Hud_ModsText(dst, 4, 12, CP_Status() == kCpOff ? "CONNECT CHAT FIRST" :
                             CP_Status() == kCpOnline ? "REDEEM ONE TO ADD IT" :
                             "WAITING FOR TWITCH");
    Hud_ModsText(dst, 3, 23, "B BACK");
  } else {
    if (g_hud_rew_cursor >= n) g_hud_rew_cursor = n - 1;
    int page = g_hud_rew_cursor / kRewRows, first = page * kRewRows;
    for (int i = 0; i < kRewRows && first + i < n; i++) {
      int y = 6 + i * 2;
      char title[16];
      const char *t = CP_RewardTitle(first + i);
      int k = 0;
      while (t[k] != 0 && k < 15) { title[k] = t[k]; k++; }
      title[k] = 0;
      if (first + i == g_hud_rew_cursor) g_hud_mods_pal = 0x3400;   // selected: yellow
      Hud_ModsText(dst, 4, y, title);
      Hud_ModsTextRight(dst, 28, y, CP_RewardVerb(first + i));
      g_hud_mods_pal = 0x2c00;
    }
    char foot[28];
    if (n > kRewRows)
      snprintf(foot, sizeof(foot), "PAGE %d OF %d  B BACK", page + 1, (n + kRewRows - 1) / kRewRows);
    else
      snprintf(foot, sizeof(foot), "LEFT RIGHT SET  B BACK");
    Hud_ModsText(dst, 3, 23, foot);
  }

  if (filtered_joypad_H & kJoypadH_B) {
    g_hud_mods_sub = 0;
    sound_effect_2 = 32;
  } else if (n > 0 && (filtered_joypad_H & kJoypadH_Up)) {
    g_hud_rew_cursor = (g_hud_rew_cursor + n - 1) % n;
  } else if (n > 0 && (filtered_joypad_H & kJoypadH_Down)) {
    g_hud_rew_cursor = (g_hud_rew_cursor + 1) % n;
  } else if (n > 0 && (filtered_joypad_H & (kJoypadH_Left | kJoypadH_Right))) {
    CP_RewardCycle(g_hud_rew_cursor, (filtered_joypad_H & kJoypadH_Left) ? -1 : 1);
    sound_effect_2 = 32;
  }
}
...
void Hud_NormalMenu() {  // 8ddf15
  ...
  if (!BYTE(joypad1H_last))
    BYTE(hud_tmp1) = 0;
  if (Twitch_Debug()) {  // pad trace for the harness (twitch_config debug=1): raw
    static int last_l = -1, last_h = -1;   // pad bytes as the pause menu sees them
    if (joypad1L_last != last_l || joypad1H_last != last_h) {
      last_l = joypad1L_last; last_h = joypad1H_last;
      printf("[hud] pad L=%02x H=%02x fL=%02x fH=%02x page=%d\n",
             joypad1L_last, joypad1H_last, filtered_joypad_L, filtered_joypad_H, g_hud_mods_page);
      fflush(stdout);
    }
  }

  // MODS page chord: HOLD SELECT + tap L/R shoulder.  Consumes the frame so
  // Select's save/exit branch below cannot also fire mid-chord.
  if ((kMods_SelectHeld && kMods_LRchord) || (kMods_LRheld && kMods_SelectEdge) || s_hud_mods_hotkey) {
    if (Twitch_Debug())
      printf("RESULT modspage via=%s now=%d\n", s_hud_mods_hotkey ? "hotkey" : "chord", !g_hud_mods_page), fflush(stdout);
    s_hud_mods_hotkey = 0;
    g_hud_mods_page ^= 1;
    g_hud_mods_cursor = 0;
    s_hud_sel_chord = 1;
    s_hud_sel_held = 1;  // route the release through the gate below so the
                         // chord flag is consumed with it
    sound_effect_2 = 32;
    if (!g_hud_mods_page) {
      // repaint the inventory page (the MODS draw blanked over it)
      Hud_DrawYButtonItems();
      Hud_DrawAbilityBox();
      Hud_DrawProgressIcons();
      Hud_DrawEquipmentBox();
  ...
  // Allow select to open the save/exit thing - fires on RELEASE so a SELECT
  // hold can serve as the MODS-page chord modifier (SELECT+L/R) without
  // tripping the save menu first.
  if (joypad1H_last & kJoypadH_Select) {
    s_hud_sel_held = 1;
  } else if (s_hud_sel_held) {
    int chord = s_hud_sel_chord;
    s_hud_sel_held = 0;
    s_hud_sel_chord = 0;
    if (!chord && sram_progress_indicator) {
      BG3VOFS_copy2 = -8;
      Hud_CloseMenu();
      DisplaySelectMenu();
      return;
  ...
  // While the MODS page is up, the equip picker and the inventory redraws
  // below must not run - they would paint straight over the MODS page (and
  // move equipment invisibly).  The page redraws itself + routes its own
  // input every frame, like the bottle menu does.
  if (g_hud_mods_page) {
    if (g_hud_mods_sub)
      Hud_RewardsPage();
    else
...
void Hud_CloseMenu() {  // 8ddfba
  ...
  // leaving the pause menu drops any MODS page state so re-entering starts
  // on the normal inventory page; a seed edit still pending (menu closed
  // with the d-pad held) is written now so nothing edited in-game is lost
  Hud_ModsCommitSeedIfDirty();
  g_hud_mods_page = 0;
  g_hud_mods_cursor = 0;
  g_hud_mods_sub = 0;
  s_hud_sel_held = 0;
...
void Hud_RebuildIndoor() {  // 8dfa60
  ...
  // universal keys: houses and caves show the shared pool too, and must not
  // blank the counter Link is carrying around
  if (g_rando_universal_keys)
    Randomizer_UniversalKeysHold();
  else
    link_num_keys = 0xff;
  Hud_Rebuild();
}
...
static void Hud_Update_Inventory() {  // 8dfc09
  ...
  base_tile = base_tiles[link_item_bombs == Randomizer_MaxBombs()];
  ...
  // universal keys: the shared pool holds up to 29 keys and the vanilla key
  // display is a single digit, so borrow the blank tile to its left for the
  // tens.  A vanilla dungeon never holds ten keys, so this never fires when
  // the file is not universal; the row is redrawn from kHudInventoryBg above
  // every call, which blanks it again as soon as the count drops below ten.
  if (g_rando_universal_keys && link_num_keys != 0xff && link_num_keys >= 10)
    dst[HUDXY(9, 1)] = 0x2400 | d[2];

  // DUNG COUNT: seed chests still unopened in this dungeon, two digits above
  // the right end of the heart row - hud_tile_indices_buffer[HUDXY(28,0)] and
  // [HUDXY(29,0)].  Those two tiles are free in every state: Hud_Rebuild
  // paints x = 20..31 of the top row from kHudTilemapRightPart, whose only
  // non-blank entries are the heart-container art at x = 22..27, and
  // Hud_UpdateHearts_Inner only ever writes rows 1 and down.  Unlike the
  // counters in the row below they also do not move with the rupee digits'
  // inv_offs shift.  Both tiles are written on every call, blank when there
  // is nothing to show, so a stale count cannot linger after leaving a
  // dungeon (Hud_Update_Inventory runs every frame from Hud_RefillLogic).
  {
    int left = Randomizer_DungeonItemsLeft();
    uint16 lo = 0x207f, hi = 0x207f;
    if (left >= 0) {
      if (left > 99) left = 99;
      lo = 0x2400 | (uint16)(0x90 + left % 10);
      if (left >= 10)
        hi = 0x2400 | (uint16)(0x90 + left / 10);
    }
    hud_tile_indices_buffer[HUDXY(28, 0)] = hi;
    hud_tile_indices_buffer[HUDXY(29, 0)] = lo;
  }
}
