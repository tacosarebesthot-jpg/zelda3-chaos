...
static void VerifySnapshotsEq(Snapshot *b, Snapshot *a, Snapshot *prev) {
  ...
  // RATE LIMIT (2026-09-10): with a RAM-diverging feature enabled (e.g. the
  // randomizer), the compare fails EVERY frame and the dumps below fire at
  // ~7000 lines/second forever. Redirected to a file that is only wasteful;
  // with a real console attached the synchronous WriteConsole spam starves
  // the main thread and the window goes "Not Responding" from boot
  // (reproduced + root-caused 2026-09-10). Keep the first failures verbose so
  // the harness still gets its diff, then go silent. g_fail semantics are
  // unchanged - only the printing is gated - so recovery behavior is
  // identical.
  static int compare_dump_budget = 3;
  bool compare_verbose = compare_dump_budget-- > 0;
