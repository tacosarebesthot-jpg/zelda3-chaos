...
#include "enemizer.h"   // ENEMY COLOR: the per-file sprite palette remap
#include <stddef.h>     // ptrdiff_t (OW/UW COLORS shadow-buffer offset math)
...
// ===========================================================================
// OW COLORS / UW COLORS (the "ow_palettes" / "uw_palettes" rows) - a per-file
// seeded transform of the BACKGROUND palettes only, in the same style as
// ENEMY COLOR (enemizer.c's Enemizer_PalMainSpr/PalAux1Spr): a shadow copy of
// the base asset table, indirected through at the exact point this file
// already reads it, cached and rebuilt only when the file/seed/mode changes.
//
// OW COLORS only ever remaps kPalette_OverworldBgMain / _OverworldBgAux12 /
// _OverworldBgAux3 - all three are BACKGROUND-only tables (nothing else in
// the engine reads them; grepped) - so sprite, HUD, Link and text palettes
// are never touched.
//
// UW COLORS remaps kPalette_DungBgMain, which Palette_Load_DungeonSet below
// ALSO reuses for one sprite palette (kPal_sp0r/kPal_sp7r, the indoor "0r"
// slot - see the usage summary above): that second Palette_LoadSingle call
// deliberately keeps reading the REAL `src` pointer, never the shadow one,
// so the sprite slot always gets the true dungeon color regardless of this
// row's setting.
//
// Reference note: the actual ALttPDoorRandomizer only offers default/random/
// blackout for both rows (Rom.py randomize_ow_palettes/randomize_uw_palettes
// hand-pick named color groups per screen - far more than compile-only
// reasoning can safely reproduce here), so RANDOM here reuses ENEMY COLOR's
// own technique instead: a per-word hue rotation (rotating which of the
// three 5-bit BGR555 channels holds which value can never crush a color to
// black or white, so the result stays readable), seeded from the file.
// ===========================================================================
enum { kBgPal_Default, kBgPal_Random, kBgPal_Blackout, kBgPal_Count };

static uint32 s_bgpal_rng;
static uint32 BgPalRndNext(void) {
  uint32 x = s_bgpal_rng;
  x ^= x << 13;
  x ^= x >> 17;
  x ^= x << 5;
  return s_bgpal_rng = x;
}
static void BgPalRndSeed(int seed, uint32 salt) {
  uint32 s = ((uint32)seed ^ salt) * 2654435761u + 0x9e3779b9u;
  s_bgpal_rng = s | 1;
}
static const uint8 kBgPalHueRot[6][3] = {
  {0, 1, 2}, {0, 2, 1}, {1, 0, 2}, {1, 2, 0}, {2, 0, 1}, {2, 1, 0},
};
static void BgPalHueWords(uint16 *v, size_t n) {
  for (size_t i = 0; i < n; i++) {
    const uint8 *p = kBgPalHueRot[BgPalRndNext() % 6];
    uint16 col = v[i];
    uint8 ch[3] = { (uint8)(col & 31), (uint8)(col >> 5 & 31), (uint8)(col >> 10 & 31) };
    v[i] = (uint16)(ch[p[0]] | ch[p[1]] << 5 | ch[p[2]] << 10);
  }
}

static uint16 *s_ow_main, *s_ow_aux12, *s_ow_aux3, *s_uw_main;
static int s_bgpal_ow_mode = -1, s_bgpal_uw_mode = -1, s_bgpal_seed = -1;

static void BgPalFreeAll(void) {
  free(s_ow_main); s_ow_main = NULL;
  free(s_ow_aux12); s_ow_aux12 = NULL;
  free(s_ow_aux3); s_ow_aux3 = NULL;
  free(s_uw_main); s_uw_main = NULL;
}

static uint16 *BgPalBuildShadow(const uint16 *base, size_t nwords, int mode, uint32 salt, int seed) {
  if (!nwords) return NULL;
  uint16 *v = (uint16 *)malloc(nwords * sizeof(uint16));
  if (!v) return NULL;
  if (mode == kBgPal_Blackout) {
    memset(v, 0, nwords * sizeof(uint16));
  } else {
    memcpy(v, base, nwords * sizeof(uint16));
    BgPalRndSeed(seed, salt);
    BgPalHueWords(v, nwords);
  }
  return v;
}

// Rebuilds the shadow buffers only when the loaded file, its seed or either
// row's value actually changed since the last call - cheap to call from
// every Palette_Load_OW*/DungeonSet, which happen far less than once a frame.
static void BgPalEnsure(void) {
  int ow = kBgPal_Default, uw = kBgPal_Default, seed = 0;
  if (Randomizer_FileEnabled()) {
    ow = Randomizer_FileOptValue("ow_palettes");
    uw = Randomizer_FileOptValue("uw_palettes");
    if ((unsigned)ow >= (unsigned)kBgPal_Count) ow = kBgPal_Default;
    if ((unsigned)uw >= (unsigned)kBgPal_Count) uw = kBgPal_Default;
    seed = Randomizer_FileSeed();
  }
  if (ow == s_bgpal_ow_mode && uw == s_bgpal_uw_mode && seed == s_bgpal_seed)
    return;
  s_bgpal_ow_mode = ow; s_bgpal_uw_mode = uw; s_bgpal_seed = seed;
  BgPalFreeAll();
  if (ow != kBgPal_Default) {
    s_ow_main  = BgPalBuildShadow(kPalette_OverworldBgMain,  kPalette_OverworldBgMain_SIZE  / 2, ow, 0x4f574d31u, seed);  // "OWM1"
    s_ow_aux12 = BgPalBuildShadow(kPalette_OverworldBgAux12, kPalette_OverworldBgAux12_SIZE / 2, ow, 0x4f574132u, seed);  // "OWA2"
    s_ow_aux3  = BgPalBuildShadow(kPalette_OverworldBgAux3,  kPalette_OverworldBgAux3_SIZE  / 2, ow, 0x4f574233u, seed);  // "OWB3"
  }
  if (uw != kBgPal_Default) {
    s_uw_main  = BgPalBuildShadow(kPalette_DungBgMain, kPalette_DungBgMain_SIZE / 2, uw, 0x55574d34u, seed);  // "UWM4"
  }
}

static const uint16 *OwMainSrc(const uint16 *src) {
  BgPalEnsure();
  if (s_ow_main) {
    ptrdiff_t o = src - (const uint16 *)kPalette_OverworldBgMain;
    if (o >= 0 && (size_t)o < kPalette_OverworldBgMain_SIZE / 2) return s_ow_main + o;
  }
  return src;
}
static const uint16 *OwAux12Src(const uint16 *src) {
  BgPalEnsure();
  if (s_ow_aux12) {
    ptrdiff_t o = src - (const uint16 *)kPalette_OverworldBgAux12;
    if (o >= 0 && (size_t)o < kPalette_OverworldBgAux12_SIZE / 2) return s_ow_aux12 + o;
  }
  return src;
}
static const uint16 *OwAux3Src(const uint16 *src) {
  BgPalEnsure();
  if (s_ow_aux3) {
    ptrdiff_t o = src - (const uint16 *)kPalette_OverworldBgAux3;
    if (o >= 0 && (size_t)o < kPalette_OverworldBgAux3_SIZE / 2) return s_ow_aux3 + o;
  }
  return src;
}
static const uint16 *UwMainSrc(const uint16 *src) {
  BgPalEnsure();
  if (s_uw_main) {
    ptrdiff_t o = src - (const uint16 *)kPalette_DungBgMain;
    if (o >= 0 && (size_t)o < kPalette_DungBgMain_SIZE / 2) return s_uw_main + o;
  }
  return src;
}
...
void Palette_Load_SpriteMain() {  // 9bec9e
  ...
  // ENEMY COLOR (enemizer.c): SP1..SP4, the four main enemy palette rows.
  Palette_LoadMultiple(Enemizer_PalMainSpr(src), kPal_sp1to4, 14, 3);
}

void Palette_Load_Sp5L() {  // 9becc5
  const uint16 *src = kPalette_SpriteAux1 + (palette_sp5l) * 7;
  // ENEMY COLOR: SP5L / SP6L, the per-room enemy aux rows.
  Palette_LoadSingle(Enemizer_PalAux1Spr(src), kPal_sp5l, 6);
}

void Palette_Load_Sp6L() {  // 9bece4
  const uint16 *src = kPalette_SpriteAux1 + (palette_sp6l) * 7;
  Palette_LoadSingle(Enemizer_PalAux1Spr(src), kPal_sp6l, 6);
}
...
void Palette_Load_HUD() {  // 9bee52
  ...
  // HEART COLOR (randomizer.c): the heart tiles sit in the BG palette 1 half
  // this call just wrote - see the comment on Randomizer_ApplyHeartColor.
  Randomizer_ApplyHeartColor();
}

void Palette_Load_DungeonSet() {  // 9bee74
  const uint16 *src = kPalette_DungBgMain + (palette_main_indoors >> 1) * 90;
  // UW COLORS (this file, above): the BG upload only - the sprite upload
  // right below deliberately keeps reading the real `src`, never remapped.
  Palette_LoadMultiple(UwMainSrc(src), 0x42, 14, 5);
  Palette_LoadSingle(src, palette_swap_flag ? kPal_sp7r : kPal_sp0r, 6);
}

void Palette_Load_OWBG3() {  // 9beea8
  const uint16 *src = kPalette_OverworldBgAux3 + overworld_palette_aux3_bp7_lo * 7;
  // OW COLORS (this file, above).
  Palette_LoadSingle(OwAux3Src(src), 0xE2, 6);
}

void Palette_Load_OWBGMain() {  // 9beec7
  const uint16 *src = kPalette_OverworldBgMain + overworld_palette_mode * 35;
  // OW COLORS (this file, above).
  Palette_LoadMultiple(OwMainSrc(src), 0x42, 6, 4);
}

void Palette_Load_OWBG1() {  // 9beee8
  const uint16 *src = kPalette_OverworldBgAux12 + overworld_palette_aux1_bp2to4_hi * 21;
  // OW COLORS (this file, above).
  Palette_LoadMultiple(OwAux12Src(src), 0x52, 6, 2);
}

void Palette_Load_OWBG2() {  // 9bef0c
  const uint16 *src = kPalette_OverworldBgAux12 + overworld_palette_aux2_bp5to7_hi * 21;
  // OW COLORS (this file, above).
  Palette_LoadMultiple(OwAux12Src(src), 0xB2, 6, 2);
}
