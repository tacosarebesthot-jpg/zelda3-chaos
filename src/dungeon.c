...
#include "variables.h"
#include "dungeon.h"
#include "randomizer.h"  // boss shuffle: per-room header override
#include "nmi.h"
#include "hud.h"
#include "load_gfx.h"
#include "overworld.h"
...
const uint8 *GetRoomHeaderPtr(int room) {
  const uint8 *p = kDungeonRoomHeaders + kDungeonRoomHeadersOffs[room];
  // Boss shuffle needs byte 3 (the sprite sheet) to follow the boss it put in
  // the room; the asset's headers overlap, so it hands back a patched copy.
  return Randomizer_BossRoomHeader(room, p);
}
...
// stream build (hints): is |msg| one of the telepathic tile messages?
bool Dungeon_IsTeleMsg(int msg) {
  if (msg <= 0) return false;
  for (size_t i = 0; i < sizeof(kDungeonRoomTeleMsg) / sizeof(kDungeonRoomTeleMsg[0]); i++)
    if (kDungeonRoomTeleMsg[i] == msg) return true;
  return false;
}
  ...
  if (!flag_is_link_immobilized) {
    ...
    if ((dung_bg2_attr_table[pos] & 0xf0) == 0xf0 ||
      ...
      if (door_type == kDoorType_BreakableWall) {
        ...
        link_num_keys -= 1;
        // universal keys: link_num_keys IS the shared pool's live counter, so
        // the door already took from it - just keep the saved copy in step.
...
int g_last_opened_chest_record = -1;   // stream build: kDungeonRoomChests index / 3 of the chest just opened

uint8 OpenChestForItem(uint8 tile, int *chest_position) {  // 81eb66
  g_last_opened_chest_record = -1;
  static const uint16 kChestOpenMasks[] = { 0x100, 0x200, 0x400, 0x800, 0x1000, 0x2000 };
  if (tile == 0x63)
  ...
  if (loc >= 0x8000) {
    ...
    for (i = 0; i < kDungeonRoomChests_SIZE; i += 3, chest_data += 3) {
      ...
      if ((chest_room & 0x7fff) == dungeon_room_index && --chest_idx < 0) {
        data = chest_data[2];
        g_last_opened_chest_record = i / 3;
...
void Module_PreDungeon() {  // 82821e
  ...
  // universal keys: one pool for every dungeon, so nothing is reloaded here
  if (g_rando_universal_keys)
    Randomizer_UniversalKeysHold();
  else
...
void Module07_00_PlayerControl() {  // 8288de
  // stream build: the pause menu (and with it the MODS page) opens whenever
  // Link can be controlled - the vanilla flag_block_link_menu cutscene lock
  // (new-game house, Ether tablet, Trinexx) is not honoured (owner: "if you
  // can open the menu it should be openable, even in vanilla")
  if (!(flag_custom_spell_anim_active | flag_is_link_immobilized)) {
...
void SaveDungeonKeys() {  // 82a1c7
  if (g_rando_universal_keys) {
    // universal keys: the shared pool is the only counter there is, and it
    // has to be stored even outside a palace (0xff) - the per-dungeon table
    // stays untouched, and unused, for the whole file.
    Randomizer_UniversalKeysHold();
    return;
  }
