...
#include "assets.h"
#include "randomizer.h"  // universal small keys: the pool survives the overworld
#include "rando/rando_entrance.h"  // ENTRANCE SHUFFLE: door / hole / exit remap
...
void PreOverworld_LoadProperties() {  // 8283c7
  ...
  // universal keys: the pool is Link's, not the dungeon's - it keeps counting
  // (and showing on the HUD) outside, and keys found out here are kept.
...
void Module09_00_PlayerControl() {  // 82a53c
  // stream build: flag_block_link_menu not honoured (see Module07_00_PlayerControl)
  if (!(flag_custom_spell_anim_active | flag_is_link_immobilized | trigger_special_entrance)) {
...
void LoadOverworldFromDungeon() {  // 82e4a3
  ...
  if (dungeon_room_index != 0x104 && dungeon_room_index < 0x180 && dungeon_room_index >= 0x100) {
    ...
    // ENTRANCE SHUFFLE: the row is still found by the room Link is standing in
    // (the reference never rewrites a row's room either), but where it puts
    // him on the overworld can have moved.  With the shuffle off this fills
    // straight from the same kExitData_* arrays as before.
    RandoExitRecord ex;
    RandoEntrance_ExitRecord(k, &ex);
    BG1VOFS_copy2 = BG2VOFS_copy2 = BG1VOFS_copy = BG2VOFS_copy = ex.scroll_y;
    BG1HOFS_copy2 = BG2HOFS_copy2 = BG1HOFS_copy = BG2HOFS_copy = ex.scroll_x;
    link_y_coord = ex.link_y;
    link_x_coord = ex.link_x;
    map16_load_src_off = ex.vram_loc;
    map16_load_var2 = (map16_load_src_off - 0x400 & 0xf80) >> 7;
    map16_load_dst_off = (map16_load_src_off - 0x10 & 0x3e) >> 1;
    camera_y_coord_scroll_low = ex.camera_y;
    camera_y_coord_scroll_hi = camera_y_coord_scroll_low - 2;
    camera_x_coord_scroll_low = ex.camera_x;
    camera_x_coord_scroll_hi = camera_x_coord_scroll_low - 2;
    WORD(link_direction_facing) = 2;
    ow_entrance_value = ex.door_1;
    big_rock_starting_address = ex.door_2;
    overworld_area_index = overworld_screen_index = ex.ow_area;
    overworld_unk1 = ex.unknown_1;
    overworld_unk3 = ex.unknown_3;
...
void Overworld_GetPitDestination() {  // 9bb860
  ...
  which_entrance = RandoEntrance_HoleId(i);   // ENTRANCE SHUFFLE moves holes too
  byte_7E010F = 0;
}
...
after:
  ...
  // ENTRANCE SHUFFLE: one lookup, used by both the tagalong test and the
  // entrance itself - the reference rewrites this door-table byte in place, so
  // the follower rule has to follow the interior the door actually leads to.
  uint8 ent_id = RandoEntrance_DoorId(lx);
  if (!follower_dropped && (link_pose_for_item == 1 || !CanEnterWithTagalong(ent_id - 1))) {
