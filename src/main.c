...
static void Checkpoint_Tick(void);
enum { kCheckpointSlot = 20 };   // saves/save20.sav
...
static void DrawPpuFrameWithPerf() {
  ...
  Twitch_PostDraw(pixel_buffer, pitch, g_snes_width * render_scale, g_snes_height * render_scale);
  // Settings menu (F10/F12) draws LAST so it sits above the twitch effects.
  SettingsMenu_PostDraw(pixel_buffer, pitch, g_snes_width * render_scale, g_snes_height * render_scale);
  if (g_display_perf)
...
static void SDLCALL AudioCallback(void *userdata, Uint8 *stream, int len) {
  ...
  MusicPlayer_Mix((int16 *)stream_start,
                  len_total / (int)(g_audio_channels * sizeof(int16)));
...
static bool g_texture_dirty;

static bool SdlRenderer_MakeTexture(int w, int h) {
  ...
  g_texture_w = w, g_texture_h = h;
  g_texture_dirty = false;
  if (g_texture == NULL) {
...
static bool SdlRenderer_Init(SDL_Window *window) {
  ...
  return SdlRenderer_MakeTexture(g_snes_width * tex_mult, g_snes_height * tex_mult);
}

// OPTIONS page (src/options.c): push g_config's video state to the running
// game. Everything here is what main() derives from the config at boot, redone
// live: widescreen width, renderer flags, feature bits, fullscreen, window
// size, the SDL logical size and texture. Output method needs a restart.
void Video_Apply(void) {
  ...
  if (!g_window) return;
  bool want_full = g_config.fullscreen != 0;
  bool is_full = (g_win_flags & (SDL_WINDOW_FULLSCREEN_DESKTOP | SDL_WINDOW_FULLSCREEN)) != 0;
  if (want_full != is_full) {
    g_win_flags &= ~(SDL_WINDOW_FULLSCREEN_DESKTOP | SDL_WINDOW_FULLSCREEN);
    if (want_full) g_win_flags |= SDL_WINDOW_FULLSCREEN_DESKTOP;
    SDL_SetWindowFullscreen(g_window, g_win_flags & SDL_WINDOW_FULLSCREEN_DESKTOP);
    g_cursor = !want_full;
    SDL_ShowCursor(g_cursor);
  }
  if (!want_full) {
    int scale = g_config.window_scale == 0 ? 2 : IntMin(g_config.window_scale, kMaxWindowScale);
    g_current_window_scale = (uint8)scale;
    SDL_SetWindowSize(g_window, scale * g_snes_width, scale * g_snes_height);
  }
  if (g_renderer) {
    SDL_RenderSetLogicalSize(g_renderer, g_config.ignore_aspect_ratio ? 0 : g_snes_width,
                             g_config.ignore_aspect_ratio ? 0 : g_snes_height);
    g_texture_dirty = true;
  }
...
static void SdlRenderer_BeginDraw(int width, int height, uint8 **pixels, int *pitch) {
  if (g_texture_dirty || width > g_texture_w || height > g_texture_h) {
    int tex_mult = (g_ppu_render_flags & kPpuRenderFlags_4x4Mode7) ? 4 : 1;
    if (!SdlRenderer_MakeTexture(IntMax(width, g_snes_width * tex_mult), IntMax(height, g_snes_height * tex_mult)))
      return;
  }
...
#undef main
// Crash dump: any unhandled exception writes a minidump to tools/wer_dumps/
// so post-crash diagnosis has a real stack instead of guesswork (owner rule:
// diagnostic logging first). dbghelp is PRE-LOADED at startup - calling
// LoadLibrary inside the filter risks a loader-lock hang or heap fault, which
// would lose the dump exactly when it matters.
typedef BOOL (WINAPI *FnMiniDump)(HANDLE, DWORD, HANDLE, unsigned long,
                                  const void *, const void *, void *);
static HMODULE g_crash_dbghelp;
static FnMiniDump g_crash_mini;

static void CrashDumpInit(void) {
  g_crash_dbghelp = LoadLibraryA("dbghelp.dll");
  if (g_crash_dbghelp)
    g_crash_mini = (FnMiniDump)GetProcAddress(g_crash_dbghelp,
                                              "MiniDumpWriteDump");
  CreateDirectoryA("tools", NULL);            // best-effort, may exist
  CreateDirectoryA("tools/wer_dumps", NULL);
}
...
static LONG WINAPI CrashDumpFilter(EXCEPTION_POINTERS *ep) {
  ...
  snprintf(path, sizeof(path),
           "tools/wer_dumps/crash_%04d%02d%02d_%02d%02d%02d.dmp",
           st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
  {
    // crash checkpoint: the next boot restores saves/save20.sav when this exists
    HANDLE flag = CreateFileA("saves/crash.flag", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
                              FILE_ATTRIBUTE_NORMAL, NULL);
    if (flag != INVALID_HANDLE_VALUE) {
      DWORD wrote;
      WriteFile(flag, path, (DWORD)strlen(path), &wrote, NULL);
      CloseHandle(flag);
    }
  }
  ...
  if (f != INVALID_HANDLE_VALUE) {
    // MINIDUMP_EXCEPTION_INFORMATION, laid out by hand: dbghelp.h is not
    // included (the DLL is loaded by name at runtime)
    struct { DWORD ThreadId; EXCEPTION_POINTERS *ExceptionPointers; BOOL ClientPointers; } mei;
    mei.ThreadId = GetCurrentThreadId();
    mei.ExceptionPointers = ep;
    mei.ClientPointers = FALSE;
    g_crash_mini(GetCurrentProcess(), GetCurrentProcessId(), f, 0,
                 ep ? &mei : NULL, NULL, NULL);  // 0 = MiniDumpNormal
    CloseHandle(f);
  }
  return EXCEPTION_CONTINUE_SEARCH;
}
...
int main(int argc, char** argv) {
  CrashDumpInit();
  SetUnhandledExceptionFilter((LPTOP_LEVEL_EXCEPTION_FILTER)CrashDumpFilter);
  ...
  Options_LoadOverlay();   // options.ini (OPTIONS page) over zelda3.ini
  Controls_Load();   // controls.ini (OPTIONS page on the player select) overrides the Controls lines
  Twitch_Init();
  CP_Init();      // in-game channel points (uses the chat token)
  LoadAssets();
  DialogueOverride_Init();   // optional dialogue.txt next to the exe
  QuickChat_Init();          // optional quickchat.txt (extra meme lines)
  Randomizer_Init();
  LoadLinkGraphics();
  Sprite_Select_Init();
  ...
  // set up SDL
  // Keep reading the controller when the window is not focused: a streamer
  // alt-tabs to OBS/chat constantly and SDL drops pad events for unfocused
  // windows by default (keyboard still follows focus, as it should).
  ...
  if (g_config.enable_audio) {
    ...
    MusicPlayer_Init();
    MusicPlayer_SetOutputFormat(have.freq, have.channels);
  }
  ...
  if (Twitch_Debug()) printf("[input] %d joystick(s) at start%c", SDL_NumJoysticks(), 10), fflush(stdout);
  ...
  // Crash checkpoint (owner 09-12 list item "crash checkpoint, auto-restore
  // after a crash"): the last run left saves/crash.flag (CrashDumpFilter) and
  // a rolling state exists -> load it now, once.
  if (g_config.crash_checkpoint) {
    FILE *flag = fopen("saves/crash.flag", "rb");
    if (flag) {
      char dump[256] = {0};
      size_t n = fread(dump, 1, sizeof dump - 1, flag);
      dump[n] = 0;
      fclose(flag);
      remove("saves/crash.flag");
      FILE *sv = fopen("saves/save20.sav", "rb");
      if (sv) {
        fclose(sv);
        SDL_LockMutex(g_audio_mutex);
        SaveLoadSlot(kSaveLoad_Load, kCheckpointSlot);
        SDL_UnlockMutex(g_audio_mutex);
        printf("[checkpoint] the last run crashed (%s): restored saves/save20.sav\n", dump);
        Tracker_LogActivity("CHECKPOINT", "the last run crashed: back at the last checkpoint");
      } else {
        printf("[checkpoint] the last run crashed (%s) but there is no checkpoint to restore\n", dump);
      }
    }
  }
  ...
  while(running) {
    while(SDL_PollEvent(&event)) {
      ...
      case SDL_CONTROLLERDEVICEADDED:
        if (Twitch_Debug()) printf("[input] controller added: index %d%c", event.cdevice.which, 10), fflush(stdout);
        OpenOneGamepad(event.cdevice.which);
        break;
      case SDL_CONTROLLERDEVICEREMOVED:
        if (Twitch_Debug()) printf("[input] controller removed: instance %d%c", event.cdevice.which, 10), fflush(stdout);
        break;
      case SDL_WINDOWEVENT:
        if (Twitch_Debug() && (event.window.event == SDL_WINDOWEVENT_FOCUS_GAINED || event.window.event == SDL_WINDOWEVENT_FOCUS_LOST))
          printf("[input] window focus %s%c", event.window.event == SDL_WINDOWEVENT_FOCUS_GAINED ? "GAINED" : "LOST", 10), fflush(stdout);
        break;
      ...
      case SDL_CONTROLLERBUTTONUP: {
        ...
        if (Twitch_Debug() && event.type == SDL_CONTROLLERBUTTONDOWN)
          printf("[input] pad %d button %s (%d) -> %d%c", event.cbutton.which,
                 SDL_GameControllerGetStringForButton(event.cbutton.button), event.cbutton.button, b, 10), fflush(stdout);
        if (b >= 0) {
          // OPTIONS page defining pad buttons: the press is captured, never played
          if (event.type == SDL_CONTROLLERBUTTONDOWN && Controls_CapturePad(b))
            break;
      ...
      case SDL_MOUSEBUTTONDOWN:
        ...
        break;
      case SDL_KEYDOWN:
        if (Twitch_Debug() && !event.key.repeat)
          printf("[input] key %s (sym %d, mod %x)%c", SDL_GetKeyName(event.key.keysym.sym), event.key.keysym.sym, event.key.keysym.mod, 10), fflush(stdout);
        if (Controls_Capturing() && (event.key.repeat || Controls_CaptureKey(event.key.keysym.sym)))
          break;                 // OPTIONS page defining keys: captured, never played (releases still flow)
    ...
    // OAuth handoff: /twitch/save parks the new chat token on the listener
    // thread; install it on the MAIN thread (Twitch_UpdateToken is
    // main-thread-only) and burn the stack copy immediately.
    char oauth_token[128];
    if (OAuthWeb_TakeSavedToken(oauth_token, sizeof(oauth_token))) {
      Twitch_UpdateToken(oauth_token);
    ...
    Controls_NoteInputs(inputs);   // the OPTIONS test page shows this word
...
// Rolling crash checkpoint: saves/save20.sav (slot 20, no hotkey reaches it)
// every 60 s of real play (dungeon or overworld, no transition running).
// CrashDumpFilter leaves saves/crash.flag; the next boot restores this file.
static void Checkpoint_Tick(void) {
  static uint32 frames;
  if (!g_config.crash_checkpoint) return;
  if (main_module_index != 7 && main_module_index != 9) return;   // dungeon / overworld only
  if (submodule_index != 0) return;
  if (++frames < 3600) return;
  frames = 0;
  SDL_LockMutex(g_audio_mutex);
  SaveLoadSlot(kSaveLoad_Save, kCheckpointSlot);
  SDL_UnlockMutex(g_audio_mutex);
}
...
static void HandleCommand_Locked(uint32 j, bool pressed) {
  ...
  if (j <= kKeys_Load_Last) {
    ...
    case kKeys_Reset:
      ...
      break;
    case kKeys_ModsPage:
      Hud_ModsPageHotkey();   // MODS page without the pad chord (testing / keyboard players)
      break;
    case kKeys_Pause:
      g_paused = !g_paused;
      printf("[main] emulator %s (Pause key)\n", g_paused ? "PAUSED" : "resumed");
      fflush(stdout);
      break;
    case kKeys_PauseDimmed:
      g_paused = !g_paused;
      printf("[main] emulator %s (PauseDimmed key)\n", g_paused ? "PAUSED" : "resumed");
      fflush(stdout);
...
static void HandleInput(int keyCode, int keyMod, bool pressed) {
  // Settings menu shortcut: F10 toggles it (F12 is bound in randomizer.c's
  // GetAsyncKeyState hook, which deliberately bypasses this SDL gate so F12
  // works while the menu is open too). Bare key only - with shift/ctrl the
  // zelda3.ini Save/Replay F-key rows keep working. While the menu is open it
  // is MODAL: every other key is fed to it and nothing reaches the engine,
  // so menu browsing can't walk Link off a ledge (src/settings_menu.c).
  // Harness-only: players reach every setting from the pause-menu MODS page
  // (SELECT+L/R) and the file-select rows, so the keyboard overlay is gated
  // to twitch_config.txt test=1 (owner rule: nothing hotkey-only).
  if (pressed && keyMod == 0 && keyCode == SDLK_F10 && Twitch_TestMode()) {
    SettingsMenu_Toggle();
    return;
  }
  if (SettingsMenu_IsOpen()) {
    SettingsMenu_Input(keyCode, keyMod, pressed);
    return;
  }
  // Sprite library cycling: F11 = next sprite, Shift+F11 = previous
  // (src/sprites.c Sprite_Select_Cycle). Intercepted before the config
  // keymap: F11 is unbound by default (zelda3.ini only uses F1-F10), the
  // kKeys_ plumbing is config.c-owned, and no engine hotkey can collide.
  // Debounced so key-repeat while holding F11 doesn't spin the library.
  if (pressed && keyCode == SDLK_F11) {
    static uint32 last_cycle_tick;
    uint32 now = SDL_GetTicks();
    if (now - last_cycle_tick >= 150) {
      last_cycle_tick = now;
      Sprite_Select_Cycle(keyMod & KMOD_SHIFT ? -1 : 1);
    }
    return;
  }
...
static void OpenOneGamepad(int i) {
  ...
  if (Twitch_Debug())
    printf("[input] joystick %d: '%s' game-controller=%d%c", i,
           is_gc ? SDL_GameControllerNameForIndex(i) : SDL_JoystickNameForIndex(i), is_gc, 10), fflush(stdout);
  if (is_gc) {
...
static void HandleGamepadInput(int button, bool pressed) {
  if (pressed && Controls_CapturePad(button))   // trigger axes arrive here, not as button events
    return;
