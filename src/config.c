...
int FindCmdForSdlKey(SDL_Keycode code, SDL_Keymod mod) {
  ...
  int cmd = KeyMapHash_Find(key);
  // A shift key can itself be a controller button (the default map binds
  // Select to Right Shift). Holding it must not turn every other key into an
  // unbound "Shift+x" combo, or chords like SELECT+L (the pause-menu MODS
  // page) are unreachable from the keyboard. An explicit Shift+ binding
  // still wins; otherwise fall back to the bare key.
  if (cmd == 0 && (key & kKeyMod_Shift))
    cmd = KeyMapHash_Find(key & ~kKeyMod_Shift);
  return cmd;
}
...
// ---- stream build: controls.ini (src/controls.c) -------------------------
static char g_controls_line[2][512];   // [0] pad, [1] keys; empty = built-in default
static const char kDefaultKeysLine[] = "Up, Down, Left, Right, Right Shift, Return, x, z, s, a, c, v";
static const char kDefaultPadLine[] = "DpadUp, DpadDown, DpadLeft, DpadRight, Back, Start, B, A, Y, X, L1, R1";

const char *Config_ControlsLine(int kind) {
  if (kind == 1) return g_controls_line[1][0] ? g_controls_line[1] : kDefaultKeysLine;
  return g_controls_line[0][0] ? g_controls_line[0] : kDefaultPadLine;
}

// Unlink every binding whose command is one of the 12 controls, then parse
// the new lists. Entries stay allocated (the hash and the joymap are
// append-only); they are just no longer reachable from the chains.
static bool IsControlCmd(int cmd) { return cmd >= kKeys_Controls && cmd <= kKeys_Controls_Last; }

void Config_ReplaceControls(const char *keys_csv, const char *pad_csv) {
  for (int j = 0; j < 255; j++) {
    uint16 *cur = &keymap_hash_first[j];
    while (*cur) {
      KeyMapHashEnt *ent = &keymap_hash[*cur - 1];
      if (IsControlCmd(ent->cmd)) *cur = ent->next;
      else cur = &ent->next;
    }
  }
  for (int b = 0; b < kGamepadBtn_Count; b++) {
    uint16 *cur = &joymap_first[b];
    while (*cur) {
      GamepadMapEnt *ent = &joymap_ents[*cur - 1];
      if (IsControlCmd(ent->cmd)) *cur = ent->next;
      else cur = &ent->next;
    }
  }
  char buf[512];
  snprintf(buf, sizeof buf, "%s", keys_csv ? keys_csv : "");
  ParseKeyArray(buf, kKeys_Controls, 12);
  snprintf(buf, sizeof buf, "%s", pad_csv ? pad_csv : "");
  ParseGamepadArray(buf, kKeys_Controls, 12);
}
  ...
  if (section == 0) {
    for (int i = 0; i < countof(kKeyNameId); i++) {
      if (StringEqualsNoCase(key, kKeyNameId[i].name)) {
        ...
        if (i == 1) snprintf(g_controls_line[1], sizeof g_controls_line[1], "%s", value);
    ...
    for (int i = 0; i < countof(kKeyNameId); i++) {
      if (StringEqualsNoCase(key, kKeyNameId[i].name)) {
        if (i == 1) {
          ...
          snprintf(g_controls_line[0], sizeof g_controls_line[0], "%s", value);
        }
    ...
    if (StringEqualsNoCase(key, "Autosave")) {
      ...
      return true;
    } else if (StringEqualsNoCase(key, "CrashCheckpoint")) {
      return ParseBool(value, &g_config.crash_checkpoint);
...
bool Config_ParseExtraFile(const char *filename) {
  return ParseOneConfigFile(filename, 0);
}

void ParseConfigFile(const char *filename) {
  g_config.msuvolume = 100;  // default msu volume, 100%
  g_config.crash_checkpoint = true;   // stream build default: on
