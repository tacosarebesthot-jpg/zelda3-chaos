...
#include "assets.h"
#include "audio.h"
#include "randomizer.h"  // MUSIC: Randomizer_MusicDisabled; SFX SHUFFLE lives in audio.h
#include "music_player.h"  // MUSIC FOLDER: MusicPlayer_TakeOverTrack (pool takeover)
...
static void Interrupt_NMI_AudioParts_Locked() {
  // MUSIC (the "disablemusic" row): every real track/fade request this file
  // would otherwise send gets substituted with the engine's existing "pause
  // spc player" control byte (0xf0 - see ZeldaPlayMsuAudioTrack in audio.c,
  // already used there for MSU pausing). SFX below is untouched.
  if (music_control != 0 && Randomizer_MusicDisabled())
    music_control = 0xf0;

  // MUSIC FOLDER (src/music_player.c): the same substitution, decided per
  // track. The player maps the vanilla request to a pool (title / overworld /
  // darkworld / dungeon / boss / town / cave / ending) and answers true only
  // when it actually has files for it - so a bucket with no folder, a fanfare
  // we never replace, or the player switched off all fall through and the
  // SNES tune plays as it always did. MSU is a separate replacement path:
  // do not run both (see tools/MUSIC_INTEGRATION.md).
  else if (music_control != 0 && MusicPlayer_TakeOverTrack(music_control))
    music_control = 0xf0;
  else if (music_control == 0 && MusicPlayer_TakeResumeFlag() &&
           music_unk1 != 0 && music_unk1 < 0xf0)
    music_control = music_unk1;  // folder player let go: unpause the SPC
...
//      zelda_apu_write(APUI00, 0);
  ...
  // SFX SHUFFLE (the "shuffle_sfx" row): remapped only here, at the APU
  // write - see the big comment above AudioSfx_Remap1 in audio.c.
  zelda_apu_write(APUI02, AudioSfx_Remap1(sound_effect_1));
  zelda_apu_write(APUI03, AudioSfx_Remap2(sound_effect_2));
  sound_effect_1 = 0;
  sound_effect_2 = 0;
