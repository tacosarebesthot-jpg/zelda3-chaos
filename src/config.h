...
enum {
  ...
  kKeys_VolumeUp,
  kKeys_VolumeDown,
  kKeys_ModsPage,        // stream build: keyboard key that flips the pause menu to the MODS page
  kKeys_Total,
};
...
typedef struct Config {
  ...
  bool autosave;
  bool crash_checkpoint;   // stream build: rolling state save, restored after a crash
...
// Stream build (src/controls.c): the Controls line of [KeyMap] (kind 1) or
// [GamepadMap] (kind 0) as zelda3.ini gave it, or the built-in default;
// and a way to swap the 12 control bindings for new ones at runtime.
const char *Config_ControlsLine(int kind);
void Config_ReplaceControls(const char *keys_csv, const char *pad_csv);
// Parse one more ini file over the current config (options.ini, src/options.c).
