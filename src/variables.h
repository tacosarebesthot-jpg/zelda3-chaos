...
// SMALL KEYS = UNIVERSAL (randomizer): the one shared small-key pool.
// 0x7EF38B is the last slot of the 16-entry link_keys_earned_per_dungeon
// table, and the game only ever indexes slots 0..13 (palace index >> 1,
// palaces 0x00..0x1A), so vanilla never reads or writes this byte.  The
// reference randomizer picked the same byte for exactly this counter
// (ALttPDoorRandomizer InitialSram.py: 'Small Key (Universal)': [0x38B]).
// It is inside the 0x500 block that SaveGameFile / CopySaveToWRAM and the
// file copy/erase paths move, so it is saved, copied and cleared for free.
#define link_num_keys_universal (*(uint8*)(g_ram+0xF38B))
...
// stream build: slot byte 0x4E0 (unused by vanilla) = 1 when this file has its
// randomizer settings in saves/rando_slots.ini (see randomizer.c)
#define rando_save_flag (*(uint8*)(g_ram+0xF4E0))
// BOMB BAG (randomizer): 1 once this file has found a bomb capacity upgrade,
// which under the reference's `bombbag` IS the bomb bag (features.md: "Two
// bomb bags are added to the item pool (look like +10 Capacity upgrades)").
// 0x7EF4E1 is the byte after rando_save_flag in the same never-used stretch
// of the save block: the port reads or writes nothing between 0x4E0 and
// 0x4FE (a grep over src/ for 0xF4xx finds only 0xF403, 0xF405, 0xF4E0 and
// 0xF4FE), and it is inside the 0x500 block that SaveGameFile /
// CopySaveToWRAM and the file copy/erase paths move, so it is saved, copied
// and cleared for free - the same argument the universal-keys round made for
// 0xF38B.
#define link_has_bomb_bag (*(uint8*)(g_ram+0xF4E1))
// SILVER ARROWS as a SEPARATE pool item (reference `bow_mode=silvers`): the
// pool holds "Bow" and "Silver Arrows" as two items, so picking the silvers
// up before the bow must NOT hand out a bow.  The vanilla engine has no such
// split - link_item_bow alone encodes both - so "the silvers have been found"
// lives here, and the plain Bow receipt upgrades straight to the silver bow
// when it is set (Randomizer_TakeBowUpgrade, randomizer.c).  0x7EF4E2 is the
// next byte of the same never-used stretch as link_has_bomb_bag above.
#define link_has_silvers (*(uint8*)(g_ram+0xF4E2))
// TRIFORCE HUNT (reference `goal=triforcehunt`): how many "Triforce Piece"
// items this file has collected.  The reference keeps the same count in the
// ROM's own goal-item counter, which this port does not have, so 0x7EF4E3
// (the byte after link_has_silvers) holds it - inside the 0x500 save block,
// so SaveGameFile / CopySaveToWRAM and the file copy/erase paths carry it for
// free.  Win condition: Randomizer_TakeTriforcePiece (randomizer.c).
#define link_triforce_pieces (*(uint8*)(g_ram+0xF4E3))
